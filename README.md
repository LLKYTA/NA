...
- **联系方式**：长期开放，欢迎加入；QQ 交流群（群号 980829992）；管理团队展示（服主 / 管理员）
...
QQ 不支持仅凭群号调用 API 加群，需要管理员在 [qun.qq.com/join.html](http://qun.qq.com/join.html)（群主/管理员权限）生成一键加群链接后，填入 `js/config.js`：

```js
contact: {
  qqGroup: {
    number: '980829992',
    joinLink: 'https://qm.qq.com/q/xxxxx', // 管理员生成的加群链接
  },
},
```

`joinLink` 未填写时，联系页按钮自动退化为「复制群号」；填写后变「一键加群」。
同时请确认群设置为「公开群」并允许通过链接加群。
